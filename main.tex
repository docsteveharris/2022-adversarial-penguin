...
\def\Authors{Steve Harris\,$^{1,3*}$, Thomas Keen\,$^{1}$ Watjana Lilaonitkul\,$^{1}$ Tim Bonnici \,$^{1,3}$ and Nel Swanepoel\,$^{2}$}
...
$^{2}$Centre for Advanced Research Computing, University College London, UK \\
$^{3}$Department of Critical Care, University College London Hospital, UK \\
}
...
\tiny
 \keyFont{ \section{Keywords:} translational medicine, machine learning, health informatics, ml-ops, safety, artificial intelligence} %All article types: you may provide up to 8 keywords; at least 5 are mandatory.
\end{abstract}
